...
int main(int argc, char *argv[])
{
    ...
    int compare_index = 0;
    bool correct = true;
    long long unsigned int second_factor;
    unsigned int lcm;

    memset(monkeys, 0, sizeof(monkeys));
    strcpy(file_name, "p11-example-input.txt");
    ...
    if ((file = fopen(file_name, "r")) != NULL) {
        ...
        for (round = 1; round <= 10000 && correct == true; round++) {
            ...
            if (compare_round[compare_index] == round) {
                printf("Comparing round %d\n", round);
                for (i = 0; i < 4; i++) {
                    printf("[%d] (Actual): %.8d <==> %.8d (Expected)\n", i, monkeys[i]->inspections, compare_monkey_inspections[compare_index][i]);
                    if (monkeys[i]->inspections != compare_monkey_inspections[compare_index][i]) {
                        correct = false;
                    }
                }
                if (correct == false)
